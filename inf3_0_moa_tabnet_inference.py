# %% [markdown]
# # For Train+Inference of this kernel please Visit this kernel --> https://www.kaggle.com/kushal1506/moa-tabnet-train-inference/data?scriptVersionId=47066154
...
qt = pickle.load(open('../input/moa-tabnet-correct-b8d907/qt.pkl', 'rb')) ##1
data2 = qt.transform(data[GENES+CELLS])
...
gpca= load(open('../input/moa-tabnet-correct-b8d907/gpca.pkl', 'rb'))  ##2
...
cpca = load(open('../input/moa-tabnet-correct-b8d907/cpca.pkl', 'rb'))  ##3
...
def fe_cluster_genes(train, test, n_clusters_g = 22, SEED = 42):
    ...
    def create_cluster(train, test, features, kind = 'g', n_clusters = n_clusters_g):
        ...
        kmeans_genes = load(open('../input/moa-tabnet-correct-b8d907/kmeans_genes.pkl', 'rb'))  ##4
...
def fe_cluster_cells(train, test, n_clusters_c = 4, SEED = 42):
    ...
    def create_cluster(train, test, features, kind = 'c', n_clusters = n_clusters_c):
        train_ = train[features].copy()
        test_ = test[features].copy()
        kmeans_cells = load(open('../input/moa-tabnet-correct-b8d907/kmeans_cells.pkl', 'rb'))  ##5
...
def fe_cluster_pca(train, test,n_clusters=5,SEED = 42):
        kmeans_pca = load(open('../input/moa-tabnet-correct-b8d907/kmeans_pca.pkl', 'rb'))  ##6
...
for s in SEED:
    ...
    for fold_nb, (train_idx, val_idx) in enumerate(mskf.split(train, target)):
        ...
        ### Predict on test ###
        model.load_model(f"../input/moa-tabnet-correct-b8d907/TabNet_seed_{s}_fold_{fold_nb+1}.zip")  ##7
